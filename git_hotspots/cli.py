...
Usage:
    git-hotspots [repo_path ...] [options]      # after `pip install .`
...
from git_hotspots import __version__
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  git-hotspots                            # analyze current directory
  git-hotspots /path/to/repo              # analyze specific repo
  git-hotspots repo-a repo-b repo-c       # compare multiple repos (portfolio mode)
  git-hotspots --days 30 --top 20         # last 30 days, show top 20
  git-hotspots --html report.html         # also generate HTML report
  git-hotspots --no-todos                 # skip TODO debt analysis (faster)
  git-hotspots --exclude tests --exclude "*.min.js"   # ignore paths
  git-hotspots --json                     # output JSON for scripting

"python main.py" works the same from a checkout without installing.
        """,
    )
    parser.add_argument(
